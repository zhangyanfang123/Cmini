...
void Lex::LexShow(Token *token,bool outOpt)
{
    ...
    if(token->type==ID)
    {
       ...
       // s=s+"       "+"标识符          "+id->name+"\n";
        //s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"标识符          "+id->name+endl;
    ...
    {
         //s=s+"       "+"数字常量         "+id->name+"\n";
        //s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"数字常量         "+id->name+endl;
    ...
    {
         //s=s+"       "+"字符常量         "+id->name+"\n";
       // s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"字符常量        "+id->name+endl;
    ...
    {
        //s=s+"       "+"字符串常量         "+id->name+"\n";
       //s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"字符串常量      "+id->name+endl;
    ...
    {
        //s=s+"       "+"界符               "+id->name+"\n";
        //s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"界符            "+id->name+endl;
        cout<<setw(8)<<setiosflags(ios::left)<<number;
        cout<<setw(8)<<setiosflags(ios::left)<<lex->getLineNum();
        cout<<setw(8)<<setiosflags(ios::left)<<lex->getColNum();
    ...
    {
        //s=s+"       "+"关键字          "+id->name+"\n";
        //s=s+tostring(number)+"       "+tostring(lex->getLineNum())+"       "+tostring(lex->getColNum())+"       "+"关键字          "+id->name+endl;
        cout<<setw(8)<<setiosflags(ios::left)<<number;
        cout<<setw(8)<<setiosflags(ios::left)<<lex->getLineNum();
        cout<<setw(8)<<setiosflags(ios::left)<<lex->getColNum();
...
{
    ofstream outFile;
    outFile.open("..\\demo\\mid\\Lex.txt",ios::app);
    Id *id = new Id("");
    Char *ch = new Char(' ');
    Num *num = new Num(0);
    Str *str = new Str("");
    ...
    {
        id = static_cast<Id*>(token);
        outFile<<setw(6)<<setiosflags(ios::left)<<number;
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"identifier";
    ...
    else if(token->type==NUM)
    {
        ...
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"numeric constant";
    ...
    else if(token->type==CH)
    {
       ch = static_cast<Char*>(token);
        ...
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"character constant";
        outFile<<ch->ch<<endl;
    }
    else if(token->type==STR)
    {
       str = static_cast<Str*>(token);
        ...
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"string constant";
        outFile<<getRawString(str->str)<<endl;
    }
    else  if(token->type<=58&&token->type>=25)
    {
        outFile<<setw(6)<<setiosflags(ios::left)<<number;
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"Delimiters";
        outFile<<TokenTypeArray[token->type]<<endl;
    }
    else if(token->type!=ERR)
       {
        outFile<<setw(6)<<setiosflags(ios::left)<<number;
        outFile<<setw(6)<<setiosflags(ios::left)<<row;
        outFile<<setw(6)<<setiosflags(ios::left)<<col;
        outFile<<setw(20)<<setiosflags(ios::left)<<"keyword";
        outFile<<TokenTypeArray[token->type]<<endl;
       }
       number++;
       if(outOpt==1)
...
void Lex::LexShow(bool outOpt)
{
    /*if(outOpt==true)
    {
        ofstream out;
        out.open(lexFile);
        out<<"number\t"<<"row\t"<<"column\t"<<"mark type\t"<<"mark"<<endl;
    }*/
    ofstream outFile;//addddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
    outFile.open("..\\demo\\mid\\Lex.txt",ios::ate);//addddddddddddddddddddddddddddddd
    outFile<<setw(6)<<setiosflags(ios::left)<<"num";
    outFile<<setw(6)<<setiosflags(ios::left)<<"row";
    outFile<<setw(6)<<setiosflags(ios::left)<<"col";
    outFile<<setw(20)<<setiosflags(ios::left)<<"marktype";
    outFile<<"mark"<<endl;
   ...
   //outFile.close();//adddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd
}
void Lex::analysis()
{
