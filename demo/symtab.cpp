...
void SymTab::ShowIR(bool outOpt)
{
    ...
    ofstream outFile2;
    outFile2.open("..\\demo\\mid\\Intercode.txt",ios::out);
    if(outOpt==1)
    ...
    for(funIt=funTab.begin(); funIt!=funEnd; ++funIt)
    {
        ...
        for(int j=0; j<intercode_copy.size(); j++)
        {
            if(intercode_copy[j]->label!="")
                outFile2<<intercode_copy[j]->label<<":"<<endl;
                //cout<<intercode_copy[j]->label<<":"<<endl;
            else
            {
                outFile2<<OperatorArray[int(intercode_copy[j]->op)]<<" ";//cout<<(int)(intercode_copy[j]->op)<<" ";
                if(intercode_copy[j]->target!=NULL)
                {
                    if(intercode_copy[j]->target->label!="")outFile2<<intercode_copy[j]->target->label<<" ";
                }
                if(intercode_copy[j]->result!=NULL){
                    if(intercode_copy[j]->result->name[0]=='<') outFile2<<intercode_copy[j]->result->intVal<<" ";
                    else  outFile2<<intercode_copy[j]->result->name<<" ";
                }
                if(intercode_copy[j]->arg1!=NULL)
                {
                    if(intercode_copy[j]->arg1->name[0]=='<') outFile2<<intercode_copy[j]->arg1->intVal<<" ";
                    else outFile2<<intercode_copy[j]->arg1->name<<" ";
                }
                if(intercode_copy[j]->arg2!=NULL)
                {
                    if(intercode_copy[j]->arg2->name[0]=='<') outFile2<<intercode_copy[j]->arg2->intVal<<" ";
                    else outFile2<<intercode_copy[j]->arg2->name<<" ";
                }
                if(intercode_copy[j]->fun!=NULL) outFile2<<intercode_copy[j]->fun->name<<" ";
                outFile2<<endl;
            }
        }
    }
